...
> 圖文內容維護模式的產品、Phase 狀態與技術決策見 [`docs/VISUAL_CONTENT_MODE_DESIGN.md`](docs/VISUAL_CONTENT_MODE_DESIGN.md)。Phase 1 與 Phase 2 已開放：可建立「圖文系列」、維護文字與私人圖片，並在共用發布頁管理平台標題、預覽／複製、圖片上傳順序、狀態與發布紀錄。
...
圖文在成功保存且設為「可發布」後，會加入共用發布清單；發布管理統一從主導覽或作品頁進入。圖文與長文共用發布篩選與繼續發布邏輯，但圖片不會被複製或上傳到平台；預覽會保留封面、圖說、替代文字與手動上傳順序。
