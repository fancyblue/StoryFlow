...
(function () {
  ...
  // Settings is a utility rather than a content destination, so desktop reaches it
  // from the sidebar footer and only the phone promotes it into the bottom bar,
  // where no persistent utility row exists. Which width shows which control is a
  // breakpoint question, so the stylesheets own it. Do not set `hidden` here: that
  // attribute is a hard global rule, and the previous code only appeared to work on
  // phones because a mobile display rule happened to override it.
