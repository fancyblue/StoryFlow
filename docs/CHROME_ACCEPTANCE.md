...
- [ ] Compare longform “作品與章節” and visual “作品與圖文” at the same viewport. Confirm both rails are unframed columns parted from their subject by one hairline — no card fill, border or radius on either side of it — and that outer rail width, gap and editor alignment are identical. Narrow past 820 px and confirm the hairline turns to the bottom edge of the rail in both, and that the stacked rail ends above SMART SPLIT rather than running into it. Confirm both rails head their list with an eyebrow, name and count (“CHAPTERS 章節清單 N 章”, “ENTRIES 圖文清單 N 則”), and in longform that the SOURCE and SMART SPLIT heading rules form one line.
- [ ] In each content mode, open “切換作品”. Confirm it opens as a panel under the button rather than inside the rail, that 作品名稱 below it neither moves nor widens, and that it lists “＋ 新增作品” first with the same shape in both modes.
...
- [ ] In the visual workspace, confirm the editor has no card frame of its own and that the entry rail's rows are hairline-separated with a left accent on the current one. Edit the title/body/摘要 and confirm the delayed autosave reaches “已儲存”, and that Publishing shows the new summary as a read-only click-to-copy row with no 編輯 control. Import duplicate-named JPG/PNG/WebP/GIF images, set cover/alt/caption, reorder with both drag and arrow controls, reload, and confirm text plus image order remain stable without a draft/ready control.
