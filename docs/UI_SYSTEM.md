...
- Visual workbench preview is opened by an explicit “預覽圖文” button instead of occupying the editor canvas. Preview and save actions require clear spacing, and the preview dialog separates its reading surface from the closing action with a footer divider. Publishing preview presents a compact title row, collapsed copy options and the main body/image sequence first; optional summary and Hashtags sit below as simple click-to-copy rows.
