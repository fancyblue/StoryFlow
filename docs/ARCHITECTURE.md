...
> 狀態：現行架構規範（已落地，持續維護；最後同步：2026-08-30）
...
Chrome stores only the selected directory handle in the current `storyflow-connections-v1` IndexedDB. Browser startup is non-destructive: `StoryFlowIntegrations.inspectLegacyBrowserStorage()` may report the names of known legacy localStorage keys or databases, but it never reads values, deletes data or migrates automatically. The compatibility `purgeLegacyBrowserStorage()` entry point is intentionally a no-op that reports `disabled-for-data-safety`; any future cleanup requires an explicit, Recovery-backed migration and user confirmation. `src/connection/folder-session.js` requires an explicit reconnect on a cold tab, while `src/connection/quick-start.js` presents the remembered folder name and routes the reconnect through the normal workspace/settings loader. “Leave this device” removes only the current session values and remembered current directory handle through its explicit user action.
...
`StoryFlowIntegrations.supportsFolderAccess()` is the one place that answers whether this browser can connect a folder at all. Work creation was briefly allowed to proceed where it cannot, on the reasoning that requiring a folder would lock those browsers out; measurement showed the opposite — a work created there lives only in memory and is gone on reload, with no warning — so the exception only made the failure late and silent. Both cases refuse now, and only the wording differs: a desktop Chromium user is asked to connect, and everyone else is told this browser cannot. Where no folder can be reached the empty state also drops its connect button rather than offering an action that cannot succeed.

This is a capability question rather than a device one, and it must stay that way: Chrome on Android connects a folder and reads a manuscript back, which is the phone reading route in O-07. Answering by asking the browser rather than sniffing for a phone is what keeps that route open. Reading is never gated this way in any case — importing settings.json, signing in to Google, connecting a folder and reading work back out of it are all reads.

`refreshFolderStatus()` in `src/core/app.js` dispatches `storyflow:connection-changed` whenever it resolves the folder, not only when the user picks one. The handle lives in IndexedDB and is restored after the first render, so a remembered folder only becomes known then; without the announcement the surfaces that gate on a connected folder — work creation above all — would stay closed until something else happened to re-render. `chooseFolder()` therefore no longer dispatches separately, since it awaits that same call.

The Works card shows per-work publishing progress. `src/projects/projects.js` enumerates the items — chapter parts for longform, entries for visual — but asks `window.StoryFlowPublishingStatus.forPart()` whether each one is finished, so the two surfaces cannot drift into separate definitions of published. The counter degrades to no progress element when publishing has not loaded rather than guessing.
...
`src/core/shared.js` loads first and owns the helpers that are genuinely identical everywhere: `safeName`, `normalizeTitle`, `partKey` and `hasConnectedFolder`. Each of these previously existed as a per-module copy, and `partKey` had already drifted — the publishing dashboard grew a visual-entry branch the other copies never got, so it now extends the shared base instead of restating it. Admission is narrow: a helper belongs here only when it is pure, stateless and byte-identical in every copy. `clone()` is the deliberate counter-example, since three of its copies differ in whether they fall back to JSON when `structuredClone` throws, and folding them together would change behaviour rather than remove duplication.

Consumers bind it as `const { safeName } = window.StoryFlowShared;` at the top of the IIFE that uses it. Two details cost a debugging cycle when this landed and are worth knowing before touching it. A file may hold more than one top-level IIFE — `src/connection/connection-ui.js` holds two — and the binding must go in the one that actually calls the helper, because unlike the function declaration it replaces it neither hoists nor crosses IIFE boundaries. And the in-memory fixtures under `tests/*.html` load individual modules directly rather than through the manifest, so a fixture loading a consumer must load `src/core/shared.js` before it.
...
`src/projects/content-model.js` owns the content-type boundary. Missing or invalid `contentMode` values normalize to `longform`; visual works keep `visualEntries` separate from longform `chapters`. `src/projects/visual-workspace.js` owns the completed visual-series creation transaction, visual workspace, entry/image editing and basic preview. `StoryFlowIntegrations` owns fixed-ID `Works/<work>/Visual/<entry-id>/` text, metadata and assets writes; the workspace remains the project system of record. Optional visual `summary`, `hashtags` and `afterword` are publishing helpers: they are edited in Publishing, remain separate from the body, and never become required fields. `includeAfterword` records whether platform output includes the afterword. Normalized `tags` support in-memory search and classification while the user-facing hashtag string remains easy to copy.

Visual source editing has no manual draft/ready state. Title and body edits use an 800 ms debounced autosave; image mutations save immediately; switching entries, works or views flushes the pending visual write before navigation. The visual entry's `content.md`/`metadata.json` write and the authoritative `workspace.json` write are serialized before the UI reports “已儲存”. Failed writes remain dirty and expose an explicit retry action. Entry deletion and image removal stay outside routine autosave and retain confirmation plus Recovery guards. Publishing completeness is derived rather than authored: a visual entry needs a title and either body text or at least one image before it can be marked published; summary, Hashtags and afterword remain optional.

Publishing lists every work from the in-memory project store (`StoryFlowProjects.searchSnapshot()`), the same source Works and search read and the one `workspace.json` is written from; it does not re-read `workspace.json` from the folder. Visual entries join the shared Publishing queue through the publishable adapter, including platform titles, platform Hashtags, platform publication status and publication records. `platformHashtags` is keyed by platform: a missing key inherits the common `hashtags` string, a non-empty value overrides it, and an explicit empty string means that platform uses no Hashtags. These platform publishing fields, the shared afterword and its `includeAfterword` preference are persisted in both `workspace.json` and each visual entry's `metadata.json`. Destructive actions use the same row-overflow pattern and Recovery path in Workspace, Works and Publishing: `⋯ → 刪除圖文`. They reuse `StoryFlowProjectPersistence.prepareRecovery()`, preserve assets by default and create a separate Recovery artifact before removing generated visual text. The outer multi-project workspace remains schema version 2 because these fields are backward-compatible additions.
...
`src/publishing/publishing-flow.js` owns the publishing list and, inside the expanded row, the preview panel. `buildPublishPreviewPanel()` builds it and `renderPublishPreview(panel, part, platform, contentMode)` fills it; `previewPublish()` is now only a selection — it sets `selectedPartKey` and `selectedPlatformKey` and re-renders. Because exactly one row is expanded at a time, the panel's element ids stay unique in the document. Any `renderParts()` rebuilds that panel, so what must survive a rebuild (the copy options) lives in module state rather than on the element.

`#readingView` in `index.html` is the reading surface. `src/ui/workspace-interactions.js` owns its shell (open, close, the format select, and closing it when the view or the work changes); `src/split/boundary-engine.js` owns what the flow says and which of `讀稿`/`接縫` is showing, through `window.StoryFlowSetReadingViewMode`. `src/ui/preview-mode.js` treats `#readingFlow` as the `reading` group, so the surface inherits the shared rendered-Markdown contract and its 36 em measure. Nothing about it is a dialog: it is a section inside `#workspaceView`, and `publishing-flow.js`'s `ensureViewStructure()` moves it there with the topbar and the workspace grid.
