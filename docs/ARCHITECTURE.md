...
Each publishing `part` owns `afterword`, `includeAfterword`, `platformStatus` and `publicationRecords`. The afterword is stored in `workspace.json` beside publishing state, not in a source chapter draft. `publicationRecords` is keyed by platform and contains only `{ publishedAt, url }`; it is normalized on load and written into generated article metadata with the rest of the part. `src/publishing/publishing-flow.js` composes the body and optional afterword only at preview/copy/Markdown-output time. Source refresh therefore leaves afterwords and publication records unchanged, while publishing deletion removes them with their owning parts after the normal Recovery guard.
