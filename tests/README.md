...
- `browser/storyflow.spec.js` exercises the authoritative project source controller end to end with a mocked Google Docs response, verifies transactional new-work cancel/confirm behavior and the shared disclosure chevron, confirms that an empty publishing-project selection falls back to all projects without breaking the desktop action layout, and covers independent article afterwords, optional output and deletion warnings.
